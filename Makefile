...
.PHONY: all
all: vet fmt generate manifests lint test build
...
e2e-kustomize: kustomize ## Build kustomize manifests for e2e tests
	kustomize build config/default/ -o deploy/yaml/install.yaml
...
test-all: test-clean test-unit test-e2e ## Clean tests cache then run unit and e2e tests
...
# Test E2E
test-e2e:
	go test -test.v ./test/e2e/...  -covermode=atomic -coverprofile=covere2e.out
